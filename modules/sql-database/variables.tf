...
variable "public_network_access_enabled" {
  description = "Enable public network access to SQL Server (set to true for public access with service endpoints, false for private endpoint only)"
  type        = bool
  default     = false
  # Note: Validation removed to allow flexibility during development
  # For production, recommend: public_network_access_enabled = false with enable_private_endpoint = true
}
...
variable "enable_vnet_rule" {
  description = "Enable VNet rule for SQL Server access from subnet"
  type        = bool
  default     = false
}
